...
async function start ({
  ...
  // @mimas: will register router in single-spa-vue
  // app.use(router)
  // app.mount('#q-app')
}
...
createQuasarApp(createApp, quasarUserOptions)
  .then(app => {  // @mimas: this .then() is the place where initiation get behind layout rendering!
    router = app.router
    return Promise.all(
      [
      ...
      // todo @mimas: boot options are imported from quasar.conf.js
      // quasarConf.boot.map(moduleName => import(/* webpackMode: "eager" */ `boot/${moduleName}`))
...
const vueLifecycles = singleSpaVue({
  ...
  handleInstance (app) {
    // @mimas: inject quasar UI, router, store
...
export { mainStore } from 'boot/pinia'
